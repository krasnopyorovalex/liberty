...
@section('breadcrumb')
    ...
    <li><a href="{{ route('admin.door_modifications.index', $door->parent) }}">Модификации двери {{ $door->parent->name }}</a></li>
            ...
            <div class="tabbable">
                ...
                <div class="tab-content">
                    <div class="tab-pane {{ request('active') ? '' : 'active' }}" id="main">
                        <form action="{{ route('admin.door_modifications.update', ['id' => $door->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                    ...
                    <div class="tab-pane image__box-a {{ request('active') ? 'active' : '' }}" id="textures">
                        <form action="{{ route('admin.door_modifications.store.textures', ['id' => $door->id]) }}" enctype="multipart/form-data" method="post">
                            @csrf
                                    ...
                                    @foreach($door->textures as $texture)
                                        <div class="col-md-1 texture-box">
                                            <div class="texture-item">
                                                ...
                                                <button type="button"
                                                        data-href="{{ route('admin.door_modifications.destroy.texture', ['id' => $texture->id]) }}"
                                                        class="btn delete__img btn-danger btn-labeled btn-labeled-right btn-sm">
